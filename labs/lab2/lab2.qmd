...
$$
R_{transistor} = \frac{3.3V - 0.65V}{0.002A} \approx 1.3 k\Omega
$$

A resistor value of $390 \Omega$ are chosen considering the voltage and current across the LEDs. According to the dual 7-segment display datasheet, the voltage drop across the segments are about 1.8 V. The input voltage is the voltage given by the transistor, which is 2.6 V.Aiming for approximately 2 mA across the LEDs of the segments, 

$$
R = \frac{2.6V - 1.8V}{0.02A} = 400 \Omega
$$
...
![Waveform of the simulated clock divider module](images/testbench_clk_wave.png){#fig-waveseg}

![Ending statement showing no errors for clock divider module](images/testbench_clk_end.png){#fig-endseg}

Testbench results for the clock divider module `digit_selector`
:::
